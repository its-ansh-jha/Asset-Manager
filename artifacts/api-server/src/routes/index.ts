...
router.get("/site-content", async (_req, res) => {
  try {
    ...
    res.status(500).json({ message: "Unable to load site content", error });
  }
});
...
router.put("/site-content", async (req, res) => {
  try {
    ...
    res.status(400).json({ message: "Unable to save site content", error });
  }
});
...
router.post("/admissions", async (req, res) => {
  try {
    ...
    const [saved] = await db
      .update(siteContentTable)
      .set({
        data: { ...current, admissions: [admission, ...current.admissions] },
        updatedAt: new Date(),
      })
      .where(eq(siteContentTable.id, 1))
      .returning();
    ...
    res
      .status(400)
      .json({ message: "Unable to save admission enquiry", error });
  }
});
...
router.patch("/admissions/:id", async (req, res) => {
  try {
    const current = await readContent();
    const admissions = current.admissions.map((item: unknown) =>
      item &&
      typeof item === "object" &&
      "id" in item &&
    ...
    const [saved] = await db
      .update(siteContentTable)
      .set({ data: { ...current, admissions }, updatedAt: new Date() })
      .where(and(eq(siteContentTable.id, 1)))
      .returning();
    res.json(saved.data);
  } catch (error) {
    res
      .status(400)
      .json({ message: "Unable to update admission enquiry", error });
  }
});
