...
router.get("/site-content", async (_req, res) => {
  try { res.json(await readContent()); } catch (error) { res.status(500).json({ message: "Unable to load site content", error }); }
});
...
router.patch("/admissions/:id", async (req, res) => {
  try {
    ...
    const [saved] = await db.update(siteContentTable).set({ data: { ...current, admissions }, updatedAt: new Date() }).where(and(eq(siteContentTable.id, 1))).returning();
    res.json(saved.data);
